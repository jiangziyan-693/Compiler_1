...
BasicBlock::operator std::string() const {
    ...
    if (OutParam::NEED_BB_USE_IN_BR_INFO) {
        ...
        for (Use *use = (Use *)beginUse->next; use->next != nullptr; use = (Use *) use->next) {
            str += "\n" + use->user->bb->label + "      " + std::string(*use->user);
        }
        return str;
    }
    if (OutParam::ONLY_OUTPUT_PRE_SUC) {
        std::string rett = this->label + ":       ;";
        rett += "pres: ";
        ...
        rett += "; loopDeep: " + std::to_string(this->loop->loopDepth);
        return rett;
    }
...
//    return this->label + ":\t\t\t\t\t; loopDepth: " + std::to_string(loop->loopDepth) + ";\t" + std::string(*loop);
    std::string ret = this->label + "\n";
    ret += "; pres: ";
    for (BasicBlock *bb: *precBBs) {
        ret += bb->label + " ";
    }
    ...
    if (isLoopHeader) {
        //return this.label + loop.infostd::string();
        return ret + std::string(loop->infoString());
    }
    //return this.label;
    return ret;
}
